    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F5F7FA" }}>
      ...
      <ScrollView>
        <View style={{ padding: 16 }}>
          <TouchableOpacity style={{ flexDirection: "row", alignItems: "center", backgroundColor: "#fff", padding: 16, borderRadius: 16, marginBottom: 24, elevation: 4 }}>
            <View style={{ width: 60, height: 60, borderRadius: 30, backgroundColor: "#F5F7FA", justifyContent: "center", alignItems: "center", marginRight: 16 }}><User size={30} color="#007AFF" /></View>
            <View><Text style={{ fontSize: 18, fontWeight: "600" }}>John Doe</Text><Text style={{ color: "#666", marginTop: 4 }}>View Profile</Text></View>
            <ChevronRight size={20} color="#666" style={{ marginLeft: "auto" }} />
          </TouchableOpacity>
        </View>
        <SettingsSection title="APP SETTINGS">
          <SettingsItem icon={Bell} title="Notifications" subtitle="Manage your alerts and notifications" rightElement={<Switch value={notificationsEnabled} onValueChange={setNotificationsEnabled} trackColor={{ false: "#D1D1D6", true: "#007AFF" }} />} onPress={() => setNotificationsEnabled(!notificationsEnabled)} />
          <SettingsItem icon={Moon} title="Dark Mode" rightElement={<Switch value={darkModeEnabled} onValueChange={setDarkModeEnabled} trackColor={{ false: "#D1D1D6", true: "#007AFF" }} />} onPress={() => setDarkModeEnabled(!darkModeEnabled)} />
          <SettingsItem icon={Languages} title="Language" subtitle="English (US)" onPress={() => {}} />
          <SettingsItem icon={Cloud} title="Data and Storage" onPress={() => {}} />
        </SettingsSection>
        <SettingsSection title="PRIVACY & SECURITY">
          <SettingsItem icon={Shield} title="Privacy Settings" onPress={() => {}} />
          <SettingsItem icon={Smartphone} title="Connected Devices" subtitle="Manage your connected devices" onPress={() => {}} />
        </SettingsSection>
        ...
        <View style={{ padding: 16, marginBottom: 32 }}>
          <TouchableOpacity style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#FFE5E5", padding: 16, borderRadius: 16 }} onPress={handleLogout}>
            <LogOut size={20} color="#FF3B30" style={{ marginRight: 8 }} />
            <Text style={{ color: "#FF3B30", fontSize: 16, fontWeight: "600" }}>Log Out</Text>
          </TouchableOpacity>
        </View>
      </ScrollView>
    </SafeAreaView>
